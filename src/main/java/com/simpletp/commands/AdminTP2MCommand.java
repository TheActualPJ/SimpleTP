...
public class AdminTP2MCommand implements CommandExecutor, TabCompleter {

    private final SimpleTPPlugin plugin;

    public AdminTP2MCommand(SimpleTPPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 1) {
            plugin.getMessageUtil().send(player, "general.usage", Map.of("usage", "/tp2m <player>"));
            return true;
        }
        ...
        plugin.getMessageUtil().send(player, "admin.teleported-to-you", Map.of("target", target.getName()));
        return true;
    }
