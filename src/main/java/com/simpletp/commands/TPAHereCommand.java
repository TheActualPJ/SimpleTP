...
public class TPAHereCommand implements CommandExecutor, TabCompleter {
    ...
    public TPAHereCommand(SimpleTPPlugin plugin, TPAManager tpaManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 1) {
            plugin.getMessageUtil().send(player, "general.usage", Map.of("usage", "/tpahere <player>"));
            return true;
        }
        ...
        TPAManager.SendResult result = tpaManager.sendRequest(player, target, TPARequest.Type.TPAHERE);

        switch (result) {
            case SUCCESS -> {
                plugin.getMessageUtil().send(player, "tpa.request-sent",
                        Map.of("target", target.getName(), "timeout", String.valueOf(timeout)));
                plugin.getMessageUtil().send(target, "tpa.request-received-here",
